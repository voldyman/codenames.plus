...
socket.on("afkWarning", () => {
  // Response to Afk Warning
  afkWindow.style.display = "block";
  overlay.style.display = "block";
});

socket.on("afkKicked", () => {
  // Response to Afk Kick
  afkWindow.style.display = "none";
...
function updateInfo(game, team) {
  ...
  if (team !== game.turn) endTurn.disabled = true;
  // Disable end turn button for opposite team
  ...
  clueEntryDiv.style.display =
    playerRole === "spymaster" && game.clue === null && team === game.turn
      ? ""
      : "none";
  if (game.over || game.clue === null) {
    ...
    clueDisplay.innerText =
      game.clue.word +
      " (" +
      (game.clue.count === "unlimited" ? "∞" : game.clue.count) +
      ")";
  }
}
...
function updateLog(log) {
  ...
  log.forEach(logEntry => {
    ...
    if (logEntry.event === "flipTile") {
      ...
      logSpan.innerText =
        logEntry.team +
        ' team was given the clue "' +
        logEntry.clue.word +
        '" (' +
        (logEntry.clue.count === "unlimited" ? "∞" : logEntry.clue.count) +
        ")";
